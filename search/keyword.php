...
$connect = new mysqli("localhost","root","ngopi","unstuckinajaa");
			...
			INNER JOIN tag t
			ON s.tagId = t.tagId
			WHERE m.nim=s.nim AND title LIKE '%$key%'
			GROUP BY s.stuckId
				...
				<div class="row">
					<div class="col-md-12">
						...
						<?php
							if ($count <= 0) {
						?>
							Tidak ditemukan pertanyaan dengan kata kunci <b><?php echo $key?></b>
						</div> 
						<?php
							}else{
						?>
							Ditemukan <?php echo $count?> jawaban dari <b><?php echo $key?></b>
					</div>        
